...
<body class="bg-[#021908] text-[#021908] h-screen flex flex-col md:flex-row">
  ...
  <section class="flex flex-col flex-1 bg-[#FFFFFF] h-1/2 md:h-full relative">
    ...
    <button id="backBtn"
      class="md:hidden absolute top-3 left-3 z-20 flex items-center gap-1 text-[#BBCB2F] font-semibold px-3 py-1 rounded hover:bg-[#BBCB2F]/20 transition">
      <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="2"
        viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round">
        <path d="M15 18l-6-6 6-6" />
      </svg>
    ...
    <header id="chatHeader"
      class="p-5 border-b border-[#BBCB2F] flex items-center gap-4 select-none pt-12 md:pt-5">
      <img id="contactAvatar" src="" alt="Profile"
        class="w-12 h-12 rounded-full object-cover hidden md:block border border-[#BBCB2F]" />
    ...
    <form id="messageForm" class="p-4 border-t border-[#BBCB2F] flex items-center gap-3 bg-[#F9FAF8]">
      ...
      <button type="button" id="attachFileBtn" class="text-[#BBCB2F] hover:text-[#FEFA95] transition"
      ...
      <button type="button" id="attachImageBtn" class="text-[#BBCB2F] hover:text-[#FEFA95] transition"
        aria-label="Insert image" title="Attach Image">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" stroke="currentColor" stroke-width="2"
          viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round">
  ...
  <script>
    ...
    function renderContacts() {
      ...
      Object.entries(conversations).forEach(([name, convo]) => {
        ...
        contactDiv.classList.add('flex', 'items-center', 'gap-3', 'p-3', 'cursor-pointer', 'border-b', 'border-[#BBCB2F]', 'rounded-md');
    ...
    function renderMessages(name) {
      ...
      convo.messages.forEach(msg => {
        ...
        bubble.classList.add('max-w-xs', 'px-4', 'py-2', 'text-[#021908]', 'rounded-2xl', 'break-words');
        if (msg.sender === 'me') bubble.classList.add('bg-yellow-300', 'rounded-br-none');
        else bubble.classList.add('bg-green-300', 'rounded-bl-none');
        bubble.textContent = msg.text;

        const timeDiv = document.createElement('div');
        timeDiv.classList.add('text-[10px]', 'text-right', 'text-[#02190888]', 'mt-1', 'select-none');
